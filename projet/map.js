...
Promise.all([
    ...
    fetch('referentiel-gares-voyageurs.json').then(response => response.json())
])
    .then(([horairesData, data]) => {
        createMarkersWithHoraires(horairesData, data);
    })
    .catch(error => {
...
function createMarkersWithHoraires(horairesData, data) {
    data.forEach(gare => {
        if (
            gare.wgs_84 &&
            ...
            const horairesGare = horairesData.filter(
                horaire => horaire.uic === gare.uic_code
            );

            gare.horaires = horairesGare.map(horaire => ({
                jour: horaire.jour,
                horaire_normal: horaire.horaire_normal,
                horaire_ferie: horaire.horaire_ferie,
            }));
            ...
            markers.addLayer(
                L.marker([gare.wgs_84.lat, gare.wgs_84.lon])
                    .bindPopup(gare.alias_libelle_noncontraint)
                    .on('click', function () {
                        $("#tabs").tabs("option", "active", 1);

                        const horairesTab = $("#tabs-2");
                        horairesTab.empty();

                        const joursNormaux = gare.horaires
                            .filter(horaire => horaire.horaire_normal !== undefined && horaire.horaire_normal !== "")
                            .map(horaire => `<li>${horaire.jour}: ${horaire.horaire_normal}</li>`);
                        ...
                        const joursFeries = gare.horaires
                            .filter(horaire => horaire.horaire_ferie !== undefined && horaire.horaire_ferie !== "")
                            .map(horaire => `<li>${horaire.jour}: ${horaire.horaire_ferie}</li>`);
